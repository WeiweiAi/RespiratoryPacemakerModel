...
tstart=2346;%2346
tend=4000;%4000
nflow=4; % the index of air flow
flowshift=-0.066; % baseline shift;
...
save([path 'slp67xm_1.mat'],'t','x','xAux','ibeta','T','DSense','breaths','n','SpO2','tdata','flow');
...
tempq=flow; % q
tempq(tempq>=0)=1; % when q>0, q'=1;
tempq(tempq<0)=0; % when q<0, q'=0;
indexqin = find( diff(tempq)==1)+1; %start of inspiration
%% remove very shallow breathing
nnq=0;
newindexqin=[];
for nq=1:length(indexqin)-1
    flowi=flow(indexqin(nq):indexqin(nq+1));
    if max(flowi)>=0.03 %0.02
        nnq=nnq+1;
