...
fname=[path 'slp67xm_4.mat'];
load(fname);
% the segment to fit
istart=1; % Index of the first segment;
iendt=122;% Index of the last segment;122
...
iitdata=[tdata(DSense(istart):DSense(iendt))]-9.5; % The time span shift forward 9.5s to deal with the SpO2 delay
...
params0(1,1) = 0.25;% the M_o 
params0(1,2) = 4;% the Q_CO
...
x0(7)=60;
optionsfit = optimoptions('lsqcurvefit','OptimalityTolerance',1e-9,'StepTolerance',1e-9,'MaxFunctionEvaluations',1000); % for lsqcurvefit
...
save([path 'slp67xm_6.mat'],'t','x','xAux','parL','x0',...
    'ibeta','T','DSense','breaths','SpO2','tdata','flow',...
    'ialpha','itdata','iSpO2','iiSpO2','iflow','RMSEq','RMSESpO2');
